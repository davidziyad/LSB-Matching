...
def encode(image_array, pixel_ind, message_to_encode):  # flatten image to be 1D array to get pixel values
    ...
    for i in range(0, len(message) - 1, 2):
        ...
        if image_array[pixel_ind[i]] == 0 or image_array[pixel_ind[i]] == 255 or image_array[pixel_ind[i + 1]] == 0 or \
                image_array[pixel_ind[i + 1]] == 255:
            continue
...
def decode(encoded_image, original_values, indices):
    message = ''
